...
class ExpLTH(Function):
    ...
    def __init__(self, variable: Expr, hamiltonian: Function, position_vars, momentum_vars):
        ...
        h_vars = hamiltonian.free_symbols.intersection([*position_vars, *momentum_vars])
        ...
        if self.h_p_dependence:
            ...
            raise ValueError("Hamiltonian is not a function of only position or momenta.")
    ...
    def __mul__(self, F: Expr):
        ...
        return F + self.variable * self.d_hamiltonian.dot(
            Matrix([diff(F, var) if var in non_zero_vars else 0 for var in vars_to_diff])
            )
    ...
    def __pow__(self, other):
        if not isinstance(other, int):
            ...
            raise ValueError("ExpLTH raised to the power of a non-positive integer")
